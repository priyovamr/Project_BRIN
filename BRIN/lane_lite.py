...
while True :
    ...
    try:
        ...
        gray_image = cv2.cvtColor(imgWarp, cv2.COLOR_RGB2GRAY)
        canny_image = cv2.Canny(gray_image, 100, 130, L2gradient=True)
    ...
    # print(curve)
        ...
        imgStacked = stackImages(0.7, ([image, imgWarpPoints],
                                        [canny_image, imgResult]))
        cv2.imshow('ImageStack', imgStacked)
        # cv2.imshow('Hasil', imgResult)
        if cv2.waitKey(1) & 0xFF == ord('q'):
            break
    except:
        cv2.imshow('Hasil', imageN)
        pass