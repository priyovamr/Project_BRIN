...
def processImage(inpImage):
    ...
    upper_white = np.array([86, 255, 50])
    ...
    # # Threshold_abs
    # hls = cv2.cvtColor(inpImage, cv2.COLOR_RGB2HLS)
    # hsv = cv2.cvtColor(inpImage, cv2.COLOR_RGB2HSV)
    # h_channel = hls[:,:,0]
    # l_channel = hls[:,:,1]
    # s_channel = hls[:,:,2]
    # v_channel = hsv[:,:,2]
    ...
    # sobel
    gray_image = cv2.cvtColor(inpImage, cv2.COLOR_RGB2GRAY)
    ret,binary = cv2.threshold(gray_image, 0, 255, cv2.THRESH_OTSU | cv2.THRESH_BINARY_INV)
...
intialTrackBarVals = [311,439,0,634]
...
while True:
    ...
    hist, leftBase, rightBase = plotHistogram(img2)
    print(rightBase - leftBase)

    ploty, left_fit, right_fit, left_fitx, right_fitx, rect_window = slide_window_search(img2, hist)
    
    draw_info, imgWarp = general_search(img2, left_fit, right_fit)
    curveRad, curveDir = measure_lane_curvature(ploty, left_fitx, right_fitx)
    meanPts, result = draw_lane_lines(frame, img2, minverse, draw_info)
